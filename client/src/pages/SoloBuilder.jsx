...
import { ArrowLeft, ArrowRight, Upload, Sparkles, Download, Share2, Printer, Link2, RefreshCw, Users, Minus, Plus, ChevronDown } from 'lucide-react';
...
const SoloBuilder = () => {
    ...
    const [step, setStep] = useState(1); // 0=choose(done), 1=details, 2=frame, 3=generate
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-10">
                        ...
                        <div>
                            ...
                            <ul className="mt-6 space-y-2 text-sm text-hh-cream/80 font-mono">
                                ...
                                <li className="flex gap-2"><span className="text-hh-pink">●</span> Smart photo crop — zoom in to fit your face</li>
                            ...
                            <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                {filtered.map(f => (
                                    ...
                                    <button key={f.id} onClick={() => setSelectedFrame(f.id)} className={`relative bg-hh-green-deep/60 rounded-xl p-3 border-2 transition-all text-left ${selectedFrame === f.id ? 'border-hh-yellow shadow-[4px_4px_0_#ec2f89] scale-[1.02]' : 'border-hh-cream/20 hover:border-hh-cream/60'}`}>
                                        ...
                                        <FrameCard frame={f} name={name || 'Aarav'} role={role} thumb />
                            ...
                            <div className="sticky top-6 h-fit">
                                ...
                                <div className="bg-hh-cream rounded-2xl p-4 border-2 border-hh-yellow shadow-[6px_6px_0_#0a3d24]">
                                    ...
                                    <div className="bg-hh-green-deep rounded-xl p-3">
                                        ...
                                        <FrameCard frame={frame} name={name || 'Naveen Kumar'} role={role} title={title} team={team} photo={photo} zoom={zoom} builderId={builderId} />
                                    ...
                                    <div className="mt-4 border-2 border-hh-green-deep rounded-md p-3">
                                        ...
                                        <div className="flex items-center gap-2 mt-2">
                                            ...
                                            <input type="range" min="1" max="2" step="0.05" value={zoom} onChange={e => setZoom(Number(e.target.value))} className="flex-1 accent-hh-pink" />

                                            <button onClick={() => setZoom(Math.min(2, zoom + 0.1))} className="w-8 h-8 rounded-md bg-hh-yellow border-2 border-hh-green-deep flex items-center justify-center"><Plus className="w-4 h-4 text-hh-green-deep" strokeWidth={3} /></button>

                                        </div>

                                        <button onClick={() => fileRef.current?.click()} className="mt-3 w-full border-2 border-dashed border-hh-green-deep rounded-md py-2 flex items-center justify-center gap-2 text-hh-green-deep font-mono text-[10px] tracking-widest hover:bg-hh-yellow/30">
                        ...
                        {processing ? (
                            ...
                            <>
                                ...
                                <div className="flex flex-wrap justify-center gap-3 mt-8">
                                    ...
                                    <button className="btn-yellow px-5 py-2.5 rounded-md flex items-center gap-2 text-xs tracking-widest"><Download className="w-4 h-4" /> DOWNLOAD PNG</button>

                                    <button className="btn-yellow px-5 py-2.5 rounded-md flex items-center gap-2 text-xs tracking-widest"><Share2 className="w-4 h-4" /> SHARE TO X</button>

                                    <button className="btn-yellow px-5 py-2.5 rounded-md flex items-center gap-2 text-xs tracking-widest"><Printer className="w-4 h-4" /> PRINT CARD</button>

                                    <button className="btn-yellow px-5 py-2.5 rounded-md flex items-center gap-2 text-xs tracking-widest"><Link2 className="w-4 h-4" /> COPY SHARE LINK</button>
